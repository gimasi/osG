...
osg_board_SpiInstance * _osg_board_Spi_getSpiInstance(const osg_SpiId spi)
{
    ...
    {
#ifdef SPI0
        case OSG_SPI0: return SPI0;
#endif
#ifdef SPI1
        case OSG_SPI1: return SPI1;
#endif
#ifdef SPI2
        case OSG_SPI2: return SPI2;
#endif
        default:
...
uint32_t _osg_board_Spi_decodeMode(const osg_SpiMode mode)
{
    switch (mode)
    {
        case OSG_SPI_MASTER_MODE: return SPI_MODE_MASTER;
        case OSG_SPI_SLAVE_MODE: return SPI_MODE_SLAVE;
        default:
...
uint32_t _osg_board_Spi_decodePhase(const osg_SpiPhase phase)
{
    switch (phase)
    {
        case OSG_SPI_PHASE_1ST: return SPI_PHASE_1EDGE;
        case OSG_SPI_PHASE_2ND: return SPI_PHASE_2EDGE;
        default:
...
uint32_t _osg_board_Spi_decodePolarity(const osg_SpiPolarity polarity)
{
    switch (polarity)
    {
        case OSG_SPI_POLARITY_LOW: return SPI_POLARITY_LOW;
        case OSG_SPI_POLARITY_HIGH: return SPI_POLARITY_HIGH;
        default:
...
uint32_t _osg_board_Spi_decodeEndianess(const osg_SpiEndianess endianess)
{
    switch (endianess)
    {
        case OSG_SPI_MSB_FIRST: return SPI_FIRSTBIT_MSB;
        case OSG_SPI_LSB_FIRST: return SPI_FIRSTBIT_LSB;
        default:
...
uint32_t _osg_board_Spi_decodeDataSize(const osg_SpiDataSize size)
{
    switch (size)
    {
        case OSG_SPI_DATA_8_BITS: return SPI_DATASIZE_8BIT;
        case OSG_SPI_DATA_16_BITS: return SPI_DATASIZE_16BIT;
        default:
...
uint32_t _osg_board_Spi_decodeSSMode(const osg_SpiSlaveSelectMode mode)
{
    switch (mode)
    {
        case OSG_SPI_SLAVE_SELECT_INPUT: return SPI_NSS_HARD_INPUT;
        case OSG_SPI_SLAVE_SELECT_OUTPUT: return SPI_NSS_HARD_OUTPUT;
        default:
            return SPI_NSS_SOFT;
    }
}
...
void _osg_board_Spi_enableSpiClk(const osg_SpiId spi)
{
    ...
    {
#ifdef SPI0
        case OSG_SPI0: __HAL_RCC_SPI0_CLK_ENABLE(); break;
#endif
#ifdef SPI1
        case OSG_SPI1:__HAL_RCC_SPI1_CLK_ENABLE(); break;
#endif
#ifdef SPI2
        case OSG_SPI2:__HAL_RCC_SPI2_CLK_ENABLE(); break;
#endif
#ifdef SPI3
        case OSG_SPI3: __HAL_RCC_SPI3_CLK_ENABLE(); break;
#endif
#ifdef SPI4
        case OSG_SPI4: __HAL_RCC_SPI4_CLK_ENABLE(); break;
#endif
        default:
